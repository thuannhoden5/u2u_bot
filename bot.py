...
TELEGRAM_CHAT_ID = "-1003731764881"
TELEGRAM_CHANNEL_ID = "u2u_xyzchat"
TWITTER_USERNAME = "u2u_xyz"
CHECK_INTERVAL = 300
...
def load_posted_ids(): return set(json.load(open(POSTED_IDS_FILE))) if Path(POSTED_IDS_FILE).exists() else set()
def save_posted_ids(ids): open(POSTED_IDS_FILE, "w").write(json.dumps(list(ids)))

def translate_to_vietnamese(text):
    if not TRANSLATE_ENABLED or not text.strip(): return text
    vietnamese_chars = set("àáâãèéêìíòóôõùúýăđơưạảấầẩẫậắằẳẵặẹẻẽếềểễệỉịọỏốồổỗộớờởỡợụủứừửữựỳỵỷỹ")
    if sum(1 for c in text.lower() if c in vietnamese_chars) > 5: return text
    try:
        ...
        if resp.status_code == 200:
            ...
            if translated and translated.strip(): return translated.strip()
        return text
    except: return text
    ...
    try:
        ...
        for img in raw_imgs:
            if "/pic/" in img:
                ...
                if not pic_part.startswith("http"): result.append(f"https://pbs.twimg.com/{pic_part}")
            elif img.startswith("http") and "pbs.twimg.com" in img: result.append(img)
        return result
    except: return []
...
def format_message(entry, translated, source="Twitter"):
    link = entry.get("link", "")
    try: time_str = datetime.strptime(entry.get("published", ""), "%a, %d %b %Y %H:%M:%S %z").strftime("%d/%m/%Y %H:%M")
    except: time_str = entry.get("published", "")
    icon, username = ("🐦", f"@{TWITTER_USERNAME}") if source == "Twitter" else ("💬", f"@{TELEGRAM_CHANNEL_ID}")
    msg = f"{icon} <b>{username}</b>\n\n{clean_html(translated)}\n\n🕐 {time_str}"
    if link: msg += f"\n🔗 <a href='{link}'>Xem gốc</a>"
...
def fetch_telegram_messages():
    try:
        ...
        for update in updates:
            ...
            if msg and msg.get("chat", {}).get("username") == TELEGRAM_CHANNEL_ID: messages.append(msg)
        if messages: log.info(f"  → Lấy được {len(messages)} message từ Telegram")
        return messages
    except: return []
...
def format_telegram_message(msg):
    try:
        ...
        images = []
        if "photo" in msg: images = [msg["photo"][-1].get("file_id", "")]
        elif "document" in msg and msg["document"].get("mime_type", "").startswith("image"): images = [msg["document"].get("file_id", "")]
...
def send_telegram(text, images=None):
    try:
        ...
        if not img_bytes: return _check(requests.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage", json={"chat_id":TELEGRAM_CHAT_ID,"text":text,"parse_mode":"HTML"}, timeout=10))
        caption = text[:1024]
        if len(img_bytes) == 1:
        ...
        for i, img_data in enumerate(img_bytes):
            ...
            if i == 0: item.update({"caption":caption,"parse_mode":"HTML"})
            media.append(item)
...
def _check(resp):
    ...
    else: log.error(f"Telegram HTTP {resp.status_code}: {resp.text[:300]}")
    return False
    ...
    while True:
        try:
            ...
            for entry in reversed(all_items):
                ...
                if item_id in posted_ids: continue
                ...
                else: log.warning(f"❌ Gửi thất bại: {item_id}")
